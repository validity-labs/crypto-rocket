...
locals {
  ...
  environment_label = trimspace(var.environment_label) != "" ? var.environment_label : local.environment_name
}
...
# -- GCS BUCKET FOR THE TERRAFORM STATE ---------
...
resource "gitlab_project_variable" "crypto_rocket_ui_domain" {
  project           = var.gitlab_project
  key               = "TF_VAR_crypto_rocket_ui_domain"
