...
// import { CherubsDAOABI, CherubsDaoLogicV1Factory } from '@nouns/sdk';
...
// const abi = new utils.Interface(CherubsDAOABI);
// const nounsDaoContract = new CherubsDaoLogicV1Factory().attach(config.addresses.cherubsDAOProxy);
...
//               callData: types ? types : value ? `${utils.formatEther(value)} ${CURRENCY}` : '',
//             };
//           }
...
//   const { send: castVote, state: castVoteState } = useContractFunction(nounsDaoContract, 'castVote');
...
//   const { send: queueProposal, state: queueProposalState } = useContractFunction(nounsDaoContract, 'queue');
//   return { queueProposal, queueProposalState };
// };

// export const useExecuteProposal = () => {
//   const { send: executeProposal, state: executeProposalState } = useContractFunction(nounsDaoContract, 'execute');
