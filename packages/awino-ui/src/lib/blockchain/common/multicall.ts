...
export const multicallV1 = async <T = any>(abi: any[], calls: Call[], provider: any): Promise<T> => {
  const address = MULTICALL_ADDRESS_MAP[CHAIN_ID];
  const multi = new Contract(address, multicallAbiJson, provider);
 ...
 */
const multicall = async <T = any>(abi: any[], calls: Call[], provider: any, options?: MulticallOptions): Promise<T> => {
