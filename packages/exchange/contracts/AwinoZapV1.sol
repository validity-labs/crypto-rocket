...
import {IAwinoPair} from "./interfaces/IAwinoPair.sol";
import {IAwinoRouter02} from "./interfaces/IAwinoRouter02.sol";
import {IWETH} from "./interfaces/IWETH.sol";
import {Babylonian} from "./libraries/Babylonian.sol";

/*
 * @author Inspiration from the work of Zapper and Beefy.
 * Implemented and modified by AwinoSwap teams.
 */
contract AwinoZapV1 is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    // Interface for Wrapped BNB (WBNB)
    IWETH public WBNB;

    // AwinoRouter interface
    IAwinoRouter02 public AwinoRouter;

    // Maximum integer (used for managing allowance)
    uint256 public constant MAX_INT = 2**256 - 1;

    // Minimum amount for a swap (derived from AwinoSwap)
    ...
    // Address AwinoRouter
    address private AwinoRouterAddress;
     ...
     * @param _AwinoRouter: address of the AwinoRouter
     * @param _maxZapReverseRatio: maximum zap ratio
     */
    constructor(
        address _WBNBAddress,
        address _AwinoRouter,
        uint256 _maxZapReverseRatio
    ) {
        WBNBAddress = _WBNBAddress;
        WBNB = IWETH(_WBNBAddress);
        AwinoRouterAddress = _AwinoRouter;
        AwinoRouter = IAwinoRouter02(_AwinoRouter);
    ...
    {
        address token0 = IAwinoPair(_lpToken).token0();
        address token1 = IAwinoPair(_lpToken).token1();

        require(_tokenToZap == token0 || _tokenToZap == token1, "Zap: Wrong tokens");

        // Convert to uint256 (from uint112)
        (uint256 reserveA, uint256 reserveB, ) = IAwinoPair(_lpToken).getReserves();

        if (token0 == _tokenToZap) {
            swapTokenOut = token1;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveA, reserveB);
            swapAmountOut = AwinoRouter.getAmountOut(swapAmountIn, reserveA, reserveB);
        } else {
            swapTokenOut = token0;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveB, reserveA);
            swapAmountOut = AwinoRouter.getAmountOut(swapAmountIn, reserveB, reserveA);
        }
    ...
    {
        require(
            _token0ToZap == IAwinoPair(_lpToken).token0() || _token0ToZap == IAwinoPair(_lpToken).token1(),
            "Zap: Wrong token0"
        );
        require(
            _token1ToZap == IAwinoPair(_lpToken).token0() || _token1ToZap == IAwinoPair(_lpToken).token1(),
            "Zap: Wrong token1"
        );
        ...
        if (_token0ToZap == IAwinoPair(_lpToken).token0()) {
        ...
        )
    {
        ...
        if (token1 == _tokenToReceive) {
            // sell token0
            uint256 tokenAmountIn = (_lpTokenAmount * reserveA) / IAwinoPair(_lpToken).totalSupply();
            ...
            // sell token1
            uint256 tokenAmountIn = (_lpTokenAmount * reserveB) / IAwinoPair(_lpToken).totalSupply();
    ...
    function _zapIn(
        ...
        uint256[] memory swapedAmounts = AwinoRouter.swapExactTokensForTokens(
            swapAmountIn,
            _tokenAmountOutMin,
            path,
            address(this),
        ...
        (, , lpTokenReceived) = AwinoRouter.addLiquidity(
            path[0],
            path[1],
    ...
    function _zapOut(
        ...
        (uint256 amount0, uint256 amount1) = IAwinoPair(_lpToken).burn(address(this));

        require(amount0 >= MINIMUM_AMOUNT, "AwinoRouter: INSUFFICIENT_A_AMOUNT");
        require(amount1 >= MINIMUM_AMOUNT, "AwinoRouter: INSUFFICIENT_B_AMOUNT");
        ...
        // Swap tokens
        AwinoRouter.swapExactTokensForTokens(swapAmountIn, _tokenAmountOutMin, path, address(this), block.timestamp);
    ...
    function _approveTokenIfNeeded(address _token) private {
        if (IERC20(_token).allowance(address(this), AwinoRouterAddress) < 1e24) {
            // Re-approve
            IERC20(_token).safeApprove(AwinoRouterAddress, MAX_INT);
        }
    }
    ...
    function _calculateAmountToSwap(
        ...
        uint256 nominator = AwinoRouter.getAmountOut(halfToken0Amount, _reserve0, _reserve1);
        uint256 denominator = AwinoRouter.quote(
            halfToken0Amount,
    ...
    function _calculateAmountToSwapForRebalancing(
        ...
        if (sellToken0) {
            ...
            uint256 nominator = AwinoRouter.getAmountOut(token0AmountToSell, _reserve0, _reserve1);
            uint256 denominator = AwinoRouter.quote(
                token0AmountToSell,
            ...
            uint256 nominator = AwinoRouter.getAmountOut(token1AmountToSell, _reserve1, _reserve0);
