...
import { Pair, Bundle, Token, AwinoFactory, AwinoDayData, PairDayData, TokenDayData } from "../generated/schema";
import { ONE_BI, ZERO_BD, ZERO_BI, FACTORY_ADDRESS } from "./utils";

export function updateAwinoDayData(event: ethereum.Event): AwinoDayData {
  let awino = AwinoFactory.load(FACTORY_ADDRESS);
  let timestamp = event.block.timestamp.toI32();
  let dayID = timestamp / 86400;
  let dayStartTimestamp = dayID * 86400;

  let awinoDayData = AwinoDayData.load(dayID.toString());
  if (awinoDayData === null) {
  ...
  return awinoDayData as AwinoDayData;
}
